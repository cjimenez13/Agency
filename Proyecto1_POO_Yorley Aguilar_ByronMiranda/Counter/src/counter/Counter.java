...
public class Counter {
   ...
   // private Locker locker_available[];//array de lockers que posee la empresa a disposición.Relación de composición
    ...
    public String saveDeliveryEnvelope(int pID,Envelope pDeliverTypeEnvelope){
        Client client= getClient(pID);
        Locker locker= client.get_locker();
        locker.addEnvelope(pDeliverTypeEnvelope);
        locker.set_state(true);
        client.setCant_recibidos(client.get_cant_recibidos()+1);
        client.changeType();
        String notify=SendEmailNotify(client);
        return notify;
    }
    public String saveDeliveryPackage(int pID,Package pDeliverTypePackage){
        Client client= getClient(pID);
        Locker locker= client.get_locker();
    ...
    public String saveDeliveryMagazine(int pID,Magazine pDeliverTypeMagazine){
        Client client= getClient(pID); 
        Locker locker= client.get_locker();
    ...
    public String SendEmailNotify(Client client){
        String direccionCorreo=client.get_email();
        Mail correo=new Mail(direccionCorreo);
        int result=correo.enviarCorreo();
        if(result!=0){
            return "La notificacion ha sido enviada";
        }else{
            return "Ha ocurrido un error, la cuenta de correo de el usuario no es valida";
        }
    }
